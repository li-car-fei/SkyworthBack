...
module.exports = async function uploadFile(parts, file_first, catalog) {
    ...
    while ((stream = await parts()) != null) {
        ...
        file_first = file_first || String(path.basename(stream.filename)).replace(reg, '');
        // 文件名为：定义的文件名头 + 随机字符串 +.文件后缀                     path.extname识别出filename中的 / 只要后面的.zip等格式名
        let filename = file_first + '_' + Math.random().toString(36).substr(2) + path.extname(stream.filename).toLocaleLowerCase();
